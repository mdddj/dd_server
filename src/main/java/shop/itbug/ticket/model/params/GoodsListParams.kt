...
data class GoodsListParams(
     ...
     */
    @Schema(
        description = "默认为1，支持传统的页码分页方式和scroll_id分页方式，根据用户自身需求传入值。示例1：商品入库，则首次传入1，后续传入接口返回的pageid，接口将持续返回符合条件的完整商品列表，该方式可以避免入口商品重复；示例2：根据pagesize和totalNum计算出总页数，按照需求返回指定页的商品（该方式可能在临近页取到重复商品",
        defaultValue = "1"
    )
     ...
     */
    @Schema(
        description = "每页条数，默认为100，最大值200，若小于10，则按10条处理，每页条数仅支持输入10,50,100,200",
        defaultValue = "10"
    )
    ...
    var priceLowerLimit: String? = null,

    /**
     * 价格（券后价）上限
     */
    @Schema(description = "价格（券后价）上限")
    var priceUpperLimit: String? = null,

    /**
     * 最低优惠券面额
     */
    @Schema(description = "最低优惠券面额")
    var couponPriceLowerLimit: String? = null,

    /**
     * 最低佣金比率
     */
    @Schema(description = "最低佣金比率")
    var commissionRateLowerLimit: String? = null,

    /**
     * 最低月销量
     */
    @Schema(description = "最低月销量")
    var monthSalesLowerLimit: String? = null,

    /**
     * 偏远地区包邮，1-是，0-非偏远地区，不填默认所有商品
     */
    @Schema(description = "偏远地区包邮，1-是，0-非偏远地区，不填默认所有商品")
    var freeshipRemoteDistrict: String? = null,
)